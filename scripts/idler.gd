@tool
class_name Idler extends TouchObject

func _ready() -> void:
	super._ready()
	
	$idle_gremlin/AnimationPlayer.play("idle")
	arte_view.damaged.connect(on_idler_damaged)
...
func on_idler_damaged(intruder: Node3D) -> void:
	...
	var slice_a := preload("res://source_mesh/idle_gremlin_slice_a.blend").instantiate() as Node3D
	slice_a.position = Vector3.DOWN * 0.5
	var slice_b := preload("res://source_mesh/idle_gremlin_slice_b.blend").instantiate() as Node3D
	...
	a_object.apply_impulse((direction_to_intruder * randf_range(30.0, 40.0)).rotated(Vector3.UP, randf_range(-0.2, 0.2)) + Vector3(0.0, 28.0, 0.0))
	...
	b_object.apply_impulse((direction_to_intruder * randf_range(30.0, 40.0)).rotated(Vector3.UP, randf_range(-0.2, 0.2)) + Vector3(0.0, 20.0, 0.0))
